...
st.caption(
    "Classifies AI use cases, recommends controls, and generates audit artifacts."
)
...
with col1:
    ...
    st.text_area("Use case description", height=220, key="use_case")

    st.subheader("Data inputs (paste examples if helpful)")
    st.text_area("What data might be included?", height=140, key="data_inputs")
...
with col2:
    ...
    if sensitive_flags:
        st.error(
            f"Sensitive indicators detected: {', '.join(sorted(set(sensitive_flags)))}"
        )
    ...
    if st.button("Generate log + decision memo"):
        ...
        add_history(
            {
                ...
                "vendor": (
                    "Yes" if "Yes" in st.session_state["external_vendor"] else "No"
                ),
                "sensitive": (
                    ",".join(sorted(set(sensitive_flags)))
                    if sensitive_flags
                    else "None"
                ),
            }
        ...
        # Show paths (useful for debugging)
        st.write("Log:", log_path)
        st.write("Decision memo:", memo_path)

        # Download buttons (best for Streamlit Cloud)
        try:
            with open(memo_path, "r", encoding="utf-8") as f:
                memo_text = f.read()
            st.download_button(
                label="Download decision memo (.md)",
                data=memo_text,
            ...
            st.error(f"Could not prepare decision memo download: {e}")
        ...
        try:
            ...
            st.error(f"Could not prepare audit log download: {e}")
...
st.caption(
    "Prototype note: Governance outputs are deterministic and auditable; model responses are optional."
)
